...
void QIzlezniprofakturimasterKorekcija::on_pushButton_4_clicked()
{
	...
	QMessageBox msgBox;
	msgBox.setText(trUtf8("ПРОФАКТУРАТА ке биде КОРЕГИРАНА"));
    ...
    try
    {
		...
		QMessageBox msgBox;
		msgBox.setText(trUtf8("Про-Фактурата е успешно корегирана."));
		msgBox.setStandardButtons(QMessageBox::Ok);
		msgBox.setDefaultButton(QMessageBox::Ok);
		msgBox.exec();
		pressEscape();
	...
	catch (...)
	{
        ...
        QMessageBox msgBox;
        msgBox.setText(trUtf8("ПроФактурата не е корегирана !!!."));
        msgBox.setStandardButtons(QMessageBox::Ok);
        msgBox.setDefaultButton(QMessageBox::Ok);
        msgBox.exec();
    }
