...
void QIzlezniIspratniciKorekcija::on_pushButton_4_clicked()
{
	...
	QMessageBox msgBox;
	msgBox.setText(trUtf8("ИСПРАТНИЦАТА ке биде КОРЕГИРАНА"));
    ...
    try
    {
		...
		QMessageBox msgBox;
		msgBox.setText(trUtf8("Испратницата е успешно корегирана."));
		msgBox.setStandardButtons(QMessageBox::Ok);
		msgBox.setDefaultButton(QMessageBox::Ok);
		msgBox.exec();
		pressEscape();
	...
	catch (...)
	{
        ...
        QMessageBox msgBox;
        msgBox.setText(trUtf8("Испратницата не е корегирана !!!."));
        msgBox.setStandardButtons(QMessageBox::Ok);
        msgBox.setDefaultButton(QMessageBox::Ok);
        msgBox.exec();
    }
